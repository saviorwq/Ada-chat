...
if ($action) {
    ...
    if ($action === 'fetch_models') {
        ...
        $modelTypeFilter = strtolower(trim((string)($_GET['model_type'] ?? '')));
        ...
        $requestQuery = [];
        $baseUrlLower = strtolower((string)($provider['base_url'] ?? ''));
        $isHfProvider = (strpos($baseUrlLower, 'huggingface.co') !== false);
        if ($q !== '') {
            $requestQuery['search'] = $q;
        }
        if ($isHfProvider && $modelTypeFilter !== '') {
            if ($modelTypeFilter === 'image') $requestQuery['task'] = 'text-to-image';
            if ($modelTypeFilter === 'chat') $requestQuery['task'] = 'text-generation';
            if ($modelTypeFilter === 'vision') $requestQuery['task'] = 'image-text-to-text';
        }
        if (!empty($requestQuery)) {
            $url .= (strpos($url, '?') === false ? '?' : '&') . http_build_query($requestQuery);
        }
        ...
        if (!empty($allModels)) {
            if ($q !== '') {
                $allModels = array_values(array_filter($allModels, function($m) use ($q) {
                    return stripos((string)$m, $q) !== false;
                }));
            }
            if ($modelTypeFilter !== '') {
                $allModels = array_values(array_filter($allModels, function($m) use ($modelTypeFilter) {
                    return inferModelType((string)$m) === $modelTypeFilter;
                }));
            }
        }
        ...
        $truncated = false;
        if ($totalCount > $limit) {
            $allModels = array_slice($allModels, 0, $limit);
            $truncated = true;
        }
        ...
        echo json_encode([
            ...
            'returned_count' => count($allModels),
            'truncated' => $truncated
        ]);
        exit;
    }

    // 视频异步任务状态查询（统一代理，前端只需提供 model + task_id）
    if ($action === 'video_task_status') {
        $input = json_decode(file_get_contents('php://input'), true);
        if (!is_array($input)) $input = [];
        $modelParam = trim((string)($input['model'] ?? $_POST['model'] ?? $_GET['model'] ?? ''));
        $taskId = trim((string)($input['task_id'] ?? $_POST['task_id'] ?? $_GET['task_id'] ?? ''));
        $pollUrl = trim((string)($input['video_poll_url'] ?? $input['poll_url'] ?? $_POST['poll_url'] ?? $_GET['poll_url'] ?? ''));
        $statusPath = trim((string)($input['video_status_path'] ?? $input['status_path'] ?? $_POST['status_path'] ?? $_GET['status_path'] ?? ''));
        $pollMethod = strtoupper(trim((string)($input['video_poll_method'] ?? $input['poll_method'] ?? $_POST['poll_method'] ?? $_GET['poll_method'] ?? 'GET')));
        $pollBodyInput = $input['video_poll_body'] ?? $input['poll_body'] ?? ($_POST['poll_body'] ?? null);

        if ($modelParam === '' || strpos($modelParam, '::') === false) {
            echo json_encode(['success' => false, 'error' => 'Invalid model format, expected providerId::modelName']);
            exit;
        }
        if ($taskId === '') {
        ...
        list($providerId, $_modelName) = explode('::', $modelParam, 2);
        ...
        $videoPath = trim((string)($provider['video_path'] ?? '/videos/generations'));
        if ($videoPath === '') $videoPath = '/videos/generations';

        $replaceTaskId = function($s) use ($taskId) {
            return str_replace('{task_id}', rawurlencode($taskId), (string)$s);
        };

        $apiUrl = '';
        if ($pollUrl !== '' && preg_match('#^https?://#i', $pollUrl)) {
            $apiUrl = $replaceTaskId($pollUrl);
        } else {
            if ($statusPath !== '') {
                $path = $replaceTaskId($statusPath);
                if ($path[0] !== '/') $path = '/' . $path;
                $apiUrl = $baseUrl . $path;
            } else {
                $basePath = rtrim($videoPath, '/');
                if ($basePath === '' || $basePath[0] !== '/') $basePath = '/' . ltrim($basePath, '/');
                $apiUrl = $baseUrl . $basePath . '/' . rawurlencode($taskId);
            }
        }

        if (!in_array($pollMethod, ['GET', 'POST'], true)) {
            $pollMethod = 'GET';
        }

        $pollBody = null;
        if (is_array($pollBodyInput)) {
            $pollBody = $pollBodyInput;
        } elseif (is_string($pollBodyInput) && trim($pollBodyInput) !== '') {
            $decoded = json_decode($pollBodyInput, true);
            if (is_array($decoded)) $pollBody = $decoded;
        }
        if ($pollMethod === 'POST' && !is_array($pollBody)) {
            $pollBody = ['task_id' => $taskId];
        }
        ...
        if ($pollMethod === 'POST') {
            ...
            curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($pollBody));
        }
        ...
        if ($resp === false || ($httpCode >= 500 && $httpCode <= 599 && $curlError)) {
            echo json_encode([
                'success' => false,
                'error' => 'poll_request_failed',
                'http' => $httpCode,
                'message' => $curlError
            ]);
            exit;
        ...
        $extractVideoUrl = function($obj) use (&$extractVideoUrl) {
            if (!is_array($obj)) return '';
            $directKeys = ['video_url', 'output_url', 'result_url', 'download_url', 'video', 'url'];
        ...
        $doneStates = ['succeeded', 'success', 'completed', 'done', 'finished', 'ready'];
        $failedStates = ['failed', 'error', 'canceled', 'cancelled', 'rejected', 'timeout', 'expired'];
        $videoUrl = $extractVideoUrl($data);
        $hasError = isset($data['error']) && $data['error'] !== '';
        $isDone = ($videoUrl !== '') || in_array($statusText, $doneStates, true);
        $isFailed = $hasError || in_array($statusText, $failedStates, true);
...
$videoDuration = $input['video_duration'] ?? null;
$videoFps = $input['video_fps'] ?? null;
$videoNumFrames = $input['video_num_frames'] ?? null;
$videoSize = $input['video_size'] ?? null;
$videoWidth = $input['video_width'] ?? null;
$videoHeight = $input['video_height'] ?? null;
...
switch ($task) {
    ...
    case 'video':
        ...
        $videoMode = in_array($mode, ['text2video', 'img2video'], true) ? $mode : 'text2video';
        $postData = [
            'model' => $modelName,
            'prompt' => $prompt,
            'mode' => $videoMode
        ...
        if ($videoMode === 'img2video') {
            if (!$imageBase64) {
                http_response_code(400);
                echo json_encode(['error' => 'img2video requires image']);
                exit;
            }
            $postData['image'] = $imageBase64;
        }
        if (is_numeric($videoDuration)) {
            $postData['duration'] = max(1, min(60, (int)$videoDuration));
        }
        if (is_numeric($videoFps)) {
            $postData['fps'] = max(1, min(60, (int)$videoFps));
        }
        if (is_numeric($videoNumFrames)) {
            $postData['num_frames'] = max(1, min(1200, (int)$videoNumFrames));
        }
        if (is_string($videoSize) && preg_match('/^\d{2,5}x\d{2,5}$/', $videoSize)) {
            $postData['size'] = $videoSize;
        }
        if (is_numeric($videoWidth)) {
            $postData['width'] = max(64, min(4096, (int)$videoWidth));
        }
        if (is_numeric($videoHeight)) {
            $postData['height'] = max(64, min(4096, (int)$videoHeight));
        }
        $stream = false;
        break;
    default:
...
// 通用透传：允许客户端追加供应商私有参数（保留核心字段优先级）
$reservedKeys = [
    'task' => true, 'model' => true, 'prompt' => true, 'messages' => true, 'stream' => true,
    'temperature' => true, 'top_p' => true, 'presence_penalty' => true, 'frequency_penalty' => true,
    'max_tokens' => true, 'stop' => true, 'mode' => true, 'image' => true, 'rag' => true,
    'client' => true, 'bypassCostOptimizer' => true, 'video_duration' => true, 'video_fps' => true,
    'video_num_frames' => true, 'video_size' => true, 'video_width' => true, 'video_height' => true,
    'task_id' => true,
    'video_poll_url' => true, 'video_status_path' => true, 'video_poll_method' => true, 'video_poll_body' => true,
    'poll_url' => true, 'status_path' => true, 'poll_method' => true, 'poll_body' => true
];
if (is_array($input)) {
    foreach ($input as $k => $v) {
        $key = (string)$k;
        if (isset($reservedKeys[$key])) continue;
